...
export async function generateDashboardInsights(spendingData, month, year) {
    ...
    const categoryBreakdown = spendingData
        ...
        .join("\n");
    const userPrompt = `
You are a personal finance advisor for a college student. Analyze the following spending data for ${new Date(year, month - 1).toLocaleDateString("en-US", { month: "long", year: "numeric" })} and provide exactly 3 recommendations:
...
`;
    ...
    if (sawAvailabilityFailure) {
        throw new Error("No supported Gemini model is available for generateContent. Configure GEMINI_MODEL to a currently supported model ID.");
    }
